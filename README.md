...
2026-09-15 已完成公开发布。北京时间每天凌晨 04:00 计划自动更新一次，实际启动时间可能延迟；以页面显示的最近成功同步时间为准。
...
云端工作流位于 `.github/workflows/deploy-pages.yml`，按 UTC 每天 20:00（北京时间次日凌晨 04:00）运行，也支持 `workflow_dispatch` 和 `main` 分支 push。它在同一个工作流中执行 `npm ci`、测试、采集、构建、快照归档和 Pages 部署。成功采集后的 `site/data/events.json` 会提交回项目仓库；该提交使用 `GITHUB_TOKEN`，而部署已经在当前运行中完成，不依赖这个提交再次触发构建。采集失败时不会部署新产物，最近一次已发布的数据继续可用，同时保留本次快照 artifact 供排查。
